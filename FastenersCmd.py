...
class FSScrewObject(FSBaseObject):
    ...
    def setProperties(self, obj):
        ...
        if "type" not in propertiesList:
            obj.addProperty(
                ...
                "type",
                "Parameters",
                "Screw type").type = screwMaker.GetAllTypes(self.itemText)
            obj.type = self.type
        if "diameter" not in propertiesList:
            obj.addProperty(
                ...
                "diameter",
                "Parameters",
                "Screw diameter standard").diameter = self.diameters
        ...
        if (self.itemText == "Screw"):
            if "length" not in propertiesList:
                obj.addProperty(
                    ...
                    "length", "Parameters",
                    "Screw length")
                obj.length = screwMaker.GetAllLengths(
                    self.type, self.diameters[1])
        if (self.itemText != "Washer"):
            if "thread" not in propertiesList:
                obj.addProperty(
                    "App::PropertyBool",
                    "thread",
                    "Parameters", "Generate real thread").thread = False
        # ImportName
    ...
    def VerifyCreateMatchOuter(self, obj):
        if not (hasattr(obj, 'matchOuter')):
            obj.addProperty(
                ...
                "matchOuter",
                "Parameters",
                "Match outer thread diameter")
            obj.matchOuter = FastenerBase.FSMatchOuter
    ...
    def execute(self, fp):
        if not FreeCAD.isRestoring():
            ...
            matchouterchange = not (hasattr(self, 'matchOuter')) or \
            ...
            (key, s) = FastenerBase.FSGetKey(
                self.itemText, fp.type, d, l, threadType)
            if s == None:
            ...
            if shape != None:
                FastenerBase.FSMoveToObject(
                    fp, shape, fp.invert, fp.offset.Value)
...
FSAddScrewCommand(
    "EN1662", "EN 1662 Hexagon bolt with flange, small series", "Hex head")
FSAddScrewCommand(
    "EN1665", "EN 1665 Hexagon bolt with flange, heavy series", "Hex head")
FSAddScrewCommand(
    "ISO4762", "ISO4762 Hexagon socket head cap screw", "Hexagon socket")
FSAddScrewCommand(
    "DIN7984", "DIN 7984 Hexagon socket head cap screws with low head",
    "Hexagon socket")
FSAddScrewCommand(
    "ISO7380-1", "ISO 7380 Hexagon socket button head screw", "Hexagon socket")
FSAddScrewCommand(
    "ISO7380-2", "ISO 7380 Hexagon socket button head screws with collar",
    "Hexagon socket")
FSAddScrewCommand(
    "ISO10642", "ISO 10642 Hexagon socket countersunk head screw",
    "Hexagon socket")
FSAddScrewCommand(
    "ISO2009", "ISO 2009 Slotted countersunk flat head screw", "Slotted")
FSAddScrewCommand(
    "ISO2010", "ISO 2010 Slotted raised countersunk head screw", "Slotted")
...
FSAddScrewCommand(
    "DIN967", "DIN 967 Cross recessed pan head screws with collar", "H cross")
FSAddScrewCommand(
    "ISO7045", "ISO 7045 Pan head screws type H cross recess", "H cross")
FSAddScrewCommand(
    "ISO7046", "ISO 7046 Countersunk flat head screws H cross r.", "H cross")
FSAddScrewCommand(
    "ISO7047", "ISO 7047 Raised countersunk head screws H cross r.", "H cross")
FSAddScrewCommand(
    "ISO7048", "ISO 7048 Cheese head screws with type H cross r.", "H cross")
FSAddScrewCommand(
    "ISO14579", "ISO 14579 Hexalobular socket head cap screws",
    "Hexalobular socket")
FSAddScrewCommand(
    "ISO14580", "ISO 14580 Hexalobular socket cheese head screws",
    "Hexalobular socket")
FSAddScrewCommand(
    "ISO14582",
...
FSAddScrewCommand(
    "ISO7090", "ISO 7090 Plain Washers, chamfered - Normal series", "Washer")
FSAddScrewCommand("ISO7092", "ISO 7092 Plain washers - Small series", "Washer")
FSAddScrewCommand(
    "ISO7093-1", "ISO 7093-1 Plain washers - Large series", "Washer")
FSAddScrewCommand(
    "ISO7094", "ISO 7094 Plain washers - Extra large series", "Washer")
...
class FSWasherObject(FSBaseObject):
    def __init__(self, obj, type, attachTo):
        ...
        obj.addProperty(
            "App::PropertyEnumeration",
            "diameter",
            "Parameters", "Screw diameter standard").diameter = diameters
        obj.type = type
        obj.Proxy = self
...
class FSScrewRodObject(FSBaseObject):
    def __init__(self, obj, attachTo):
        ...
        obj.addProperty(
            "App::PropertyLength",
            "length",
            "Parameters", "Screw length").length = 20.0
        self.VerifyCreateMatchOuter(obj)
        obj.addProperty(
            "App::PropertyBool",
            "thread",
    ...
    def execute(self, fp):
        ...
        if fp.diameter == 'Auto' or matchouterchange:
            d = screwMaker.AutoDiameter(
                self.type, shape, baseobj, fp.matchOuter)
            fp.diameter = d
            diameterchange = True
        else:
            d = fp.diameter
