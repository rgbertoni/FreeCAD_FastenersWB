...
class FSScrewMaker(Screw):
    ...
    def FindClosest(self, type, diam, len):
        ...
        if not (type in screwTables):
            return (diam, len)
        name, diam_table, len_table, range_table, table_pos, k_pos = screwTables[
            type]
    ...
    def updateFastenerParameters(self):
        global FSParam
        oldState = str(self.sm3DPrintMode) + str(self.smNutThrScaleA) + str(
            self.smNutThrScaleB) + str(self.smScrewThrScaleA) + str(self.smScrewThrScaleB)
        self.sm3DPrintMode = False
        # 0 = standard, 1 = 3dprint
        ...
        newState = str(self.sm3DPrintMode) + str(self.smNutThrScaleA) + str(
            self.smNutThrScaleB) + str(self.smScrewThrScaleA) + str(self.smScrewThrScaleB)
