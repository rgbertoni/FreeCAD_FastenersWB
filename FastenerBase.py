...
class FSBaseObject:
    ...
    def __init__(self, obj, attachTo):
        obj.addProperty(
            ...
            "offset",
            "Parameters",
            "Offset from surface").offset = 0.0
        obj.addProperty(
            "App::PropertyBool",
            "invert",
            "Parameters",
            "Invert screw direction").invert = False
        obj.addProperty(
            "App::PropertyLinkSub",
            "baseObject",
            "Parameters",
...
class FSGroupCommand:
    ...
    # def Activated(self, index): # index is an int in the range [0,
    # len(GetCommands)

# and  int(FreeCAD.Version()[2].split()[0]) >= 5165
DropButtonSupported = int(float(FreeCAD.Version()[1])) > 15
# and  int(FreeCAD.Version()[2].split()[0]) >= 5560
RadioButtonSupported = int(float(FreeCAD.Version()[1])) > 15
FSParam = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fasteners")
# 0 = nine, 1 = separate toolbar 2 = drop down buttons
GroupButtonMode = FSParam.GetInt("ScrewToolbarGroupMode", 0)
    ...
    def getCommands(self, group):
        ...
        for subcommand in cmdsubs:
            if GroupButtonMode == 2:
                ...
                cmdlist.append(
                    (subcommand.replace(" ", ""), cmdsubs[subcommand]))
        return cmdlist
...
def FSAddFastenerType(typeName, hasLength=True, lengthFixed=True):
    FSFasenerTypeDB[typeName] = FSFastenerType(
...
def GetTotalObjectRepeats(obj):
    ...
    for parent in obj.InList:
        ...
        if parent.TypeId != 'App::Link' and parent.TypeId != "App::LinkElement":
            ...
            cnt += numreps * \
                (GetTotalObjectRepeats(parent) + GetNumLinks(parent))
    if cnt == 0:
        cnt = 1
    return cnt
...
class FSFaceMaker:
    ...
    def AddArc(self, x1, z1, x2, z2):
        ...
        # add an arc starting at last point, with relative center xc, zc and
        # angle a
...
class FSMatchTypeGroupCommand:
    ...
    def GetCommands(self):
        # a tuple of command names that you want to group
        return ("FSMatchTypeInner", "FSMatchTypeOuter")
    ...
    def Activated(self, index):
        ...
        if index == 0:
            FSMatchOuter = False
            FreeCAD.Console.PrintLog(
                "Set auto diameter to match inner thread\n")
        else:
            FSMatchOuter = True
            FreeCAD.Console.PrintLog(
                "Set auto diameter to match outer thread\n")

    # return the index of the tuple of the default command. This method is
    # optional and when not implemented '0' is used
...
class FSMakeBomCommand:
    ...
    def Activated(self):
        ...
        sheet = FreeCAD.ActiveDocument.addObject(
            'Spreadsheet::Sheet', 'Fasteners_BOM')
    ...
    def AddScrew(self, obj, cnt):
        self.AddFastener(obj.type + " Screw " +
